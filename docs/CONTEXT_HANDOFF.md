...
- The recommended next reliability task is adding Twilio status callbacks and sent-message persistence.
...
- Local development falls back to ignored JSON runtime files.
...
- Twilio delivery states are not persisted yet.
- Twilio status callbacks are not implemented yet.
- There is no sent-message table yet.
...
2. Add sent-message persistence.
3. Track message states such as accepted, sent, delivered, undelivered, and failed.
4. Add explicit user onboarding.
5. Route webhook events by Strava `owner_id` to the correct app user.
6. Make duplicate protection fully user-scoped after owner routing is explicit.
7. Add approved WhatsApp templates for outbound messages outside the customer service window.
8. Optionally add `activities` persistence for raw Strava activity snapshots and analysis history.
